...
//   {
//       // only update dirty boolean if it isn't already dirty if (!isDirty) 
//       { 
//           isDirty = true; isInverseDirty = true; 
//           // set all children to be dirty since any modification 
//           // of a parent transform also effects its children's 
//           // localToWorldTransform 
//           foreach (Transform child in children) { child.setDirty(); } 
//       } 
//   }
//
...
//                   // is the same as local to parent 
//                   matrix localToWorldMatrix = calculateLocalToParentMatrix(); 
//               } 
//               else 
//               { 
//                   // if there is a parent, then the localToWorldMatrix 
//                   // is calcualted recursively using the parent's localToWorldMatrix
//                   // concatenated with the local to parent 
//                   matrix localToWorldMatrix = parent.getLocalToWorldMatrix() * calculateLocalToParentMatrix(); 
//               } 
...
//           Matrix3x1 transformResult = Matrix3x1(point.x, point.y, 0) * getWorldToLocalMatrix(); return new Vector2(transformResult[1,1], transformResult[2,1], transformResult[3,1]); 
...
//       {
//           localPosition = value;
//           // set the dirty flag since the localToWorldMatrix needs to be updated setDirty(); } 
...
//    { 
//        graphics.setTransformMatrix(this.gameObject.transform.getLocalToWorldMatrix()); graphics.drawSprite(this.frame); 
//    } // whatever else the renderer does // would go here ... } 
//
//
